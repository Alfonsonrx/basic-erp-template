...
import { activateAccount, resendActivationToken } from "@actions/auth";
...
export default function AccountActivation() {
  ...
  useEffect(() => {
    if (!token) {
      setState("error");
      setErrorMessage(
        "Invalid activation link. Please check your email or request a new activation link.",
      );
      return;
    }
  ...
  const handleResend = async (e: React.FormEvent) => {
    ...
    const resendToken = async () => {
      try {
        await dispatch(resendActivationToken(resendEmail));
        setResendLoading(false);
            ...
            {!resendSuccess ? (
              ...
              <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-lg p-4">
                <p className="text-green-700 dark:text-green-400 text-sm">
                  <CheckCircle className="w-4 h-4 inline mr-2" />A new
